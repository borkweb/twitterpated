...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<header class="entry-header">
			<?php if ( is_sticky() ) : ?>
				<hgroup>
					...
					<h3 class="entry-format"><?php _e( 'Featured', 'twitterpated' ); ?></h3>
				</hgroup>
			<?php else : ?>
		...
		<div class="entry-content">
			<?php the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'twitterpated' ) ); ?>
        ...
        <?php foreach (simple_portfolio_media() as $media): ?>
				<li>
					<?php 
						if( $media['type'] == 'text' ) {
							echo do_shortcode( '[dacore-favme url="' . $media['value'] . '"]' ); 
						}//end if
					?>
				</li>
        <?php endforeach; ?>
			...
			<?php wp_link_pages( array( 'before' => '<div class="page-link"><span>' . __( 'Pages:', 'twitterpated' ) . '</span>', 'after' => '</div>' ) ); ?>
		</div><!-- .entry-content -->
		<?php endif; ?>
		...
		<footer class="entry-meta">
			...
			<?php
				...
				$categories_list = get_the_category_list( __( ', ', 'twitterpated' ) );
				if ( $categories_list ):
			?>
			<span class="cat-links">
				<?php printf( __( '<span class="%1$s">Posted in</span> %2$s', 'twitterpated' ), 'entry-utility-prep entry-utility-prep-cat-links', $categories_list );
				$show_sep = true; ?>
			</span>
			<?php endif; // End if categories ?>
			<?php
				/* translators: used between list items, there is a space after the comma */
				$tags_list = get_the_tag_list( '', __( ', ', 'twitterpated' ) );
				if ( $tags_list ):
				if ( $show_sep ) : ?>
			<span class="sep"> | </span>
				<?php endif; // End if $show_sep ?>
			<span class="tag-links">
				<?php printf( __( '<span class="%1$s">Tagged</span> %2$s', 'twitterpated' ), 'entry-utility-prep entry-utility-prep-tag-links', $tags_list );
				$show_sep = true; ?>
			</span>
			...
			<?php edit_post_link( __( 'Edit', 'twitterpated' ), '<span class="edit-link">', '</span>' ); ?>
		</footer><!-- #entry-meta -->
